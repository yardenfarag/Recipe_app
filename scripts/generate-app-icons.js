...
async function main() {
  ...
  // Icon Composer / actool often renders a 24×24 SVG as a literal speck.
  // Use a large white PNG layer so the Liquid Glass iOS icon shows the cookie.
  const iosCookie = await cookiePng(720, '#FFFFFF');
  await sharp({
    create: {
      width: 1024,
      height: 1024,
    ...
    .composite([{ input: iosCookie, gravity: 'centre' }])
    .png()
    .toFile(path.join(root, 'expo.icon', 'Assets', 'cookie.png'));
